...
for ($i = 1; $i <= $max_slider_options; $i++) {
	...
	$slider_settings_label = elgg_format_element('label', ['onclick' => '$(this).next().toggle();'], elgg_echo('widgets:image_slider:title') . ' - ' . $i);

	$slider_settings = elgg_format_element('div', [], elgg_echo('widgets:image_slider:label:url'));
	$slider_settings .= elgg_view('input/text', [
		'name' => 'params[slider_' . $i . '_url]',
		'value' => $widget->{'slider_' . $i . '_url'},
	]);

	$slider_settings .= elgg_format_element('div', [], elgg_echo('widgets:image_slider:label:text'));
	$slider_settings .= elgg_view('input/text', [
		'name' => 'params[slider_' . $i . '_text]',
		'value' => $widget->{'slider_' . $i . '_text'},
	]);

	$slider_settings .= elgg_format_element('div', [], elgg_echo('widgets:image_slider:label:link'));
	$slider_settings .= elgg_view('input/text', [
		'name' => 'params[slider_' . $i . '_link]',
		'value' => $widget->{'slider_' . $i . '_link'},
	]);

	$slider_settings .= elgg_format_element('div', [], elgg_echo('widgets:image_slider:label:direction'));
...
echo elgg_view_input('text', [
	...
	'label' => elgg_echo('widgets:image_slider:seconds_per_slide'),
...
echo elgg_view_input('text', [
	...
	'label' => elgg_echo('widgets:image_slider:slider_height'),
	'value' => $slider_height,
	'size' => 4,
	'maxlength' => 4,
]);
...
echo elgg_view_input('text', [
	...
	'label' => elgg_echo('widgets:image_slider:overlay_color'),
	'value' => $overlay_color,
	'size' => 6,
	'maxlength' => 6,
]);
